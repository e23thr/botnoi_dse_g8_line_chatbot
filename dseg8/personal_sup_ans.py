...
def Get_Personal_sup_ans(lineID):
    ...
    answer_list = list(set(
        ...
        _answer["อยากดูแลด้านไหนเป็นพิเศษ"][ans_data.iloc[0]["special1"] if ans_data.iloc[0]["special1"] != "" and ans_data.iloc[0]["special1"] is not None else "NAN"] +
        _answer["อยากดูแลด้านไหนเป็นพิเศษ"][ans_data.iloc[0]["special2"] if ans_data.iloc[0]["special2"] != "" and ans_data.iloc[0]["special2"] is not None else "NAN"] +
        _answer["อยากดูแลด้านไหนเป็นพิเศษ"][ans_data.iloc[0]["special3"] if ans_data.iloc[0]["special3"] != "" and ans_data.iloc[0]["special3"] is not None else "NAN"]
    ))
    print("*** DEBUG ***")
    print(answer_list)
